			...
			if(window.plus){
		 	 ...
		 	 mui.ajax(url,{
					...
					if(checkType(data)=='string'){
						...
						if(re && re.length==1){
							if(~re[0].replace(/\w+|\<+|\>+|\/+/gi,'').indexOf('捷众')&&!~data.indexOf('蒲公英')){
								mui.plusReady(function(){
  ...
  (function(M,$,W,D){
  	 $.extend(M,{
		...
		var arr = checkType(prev) === 'string'?$(prev):prev,key,ifI,ifT,data,reobj ;
			...
			function getV(d){
				return d[(ifI || ifI==0) && checkType(d.value)==='string'?'value':'innerText'] ;
				}
	 ...
	 */
	 	getV:function(prev,opt,tb){
	 		return M.setV(prev,$.extend(opt,{getVal:true}),opt,tb) ;
	 	},
  	 	/**
	 	 * opt.type = 1, 只是赋值文本标签
	 	 * opt.type = 2 , 只是赋值input标签
...
//							      autoBackButton:true,//左侧显示返回按钮
  	 ...
  	 mui.plusReady(function(){
	 	...
	 	if(!mui.os.ios){//添加安卓侧滑关闭功能
	 		var u =M.ws.getURL();
	 			if(!~u.indexOf('mui.indexedlist.html')&&!~['login.html','main.html','index.html','indexedlist.html','operation.html','news.html','my.html'].indexOf(u.substring(u.search(/\/www\//gi)+5))){
...
function loadNumber(type){
	function ld(el,url,Da,fn){//Da:发送ajax发送的参数,fn:ajax请求成功后的回调
		...
		ld(mui('.b-b-g-x'),'/slogin/get.do',{key:mui.os.ios?'iosupdate':'appupdate'},function(d,span){
		 //d={id:2};d=JSON.stringify(d);//测试.
			if(d!=''){
				LL.appupdateInfo = d; 
				d=JSON.parse(d);
				if(parseInt(d.id)<=parseInt(LL.appid)){
					span.remove();
				}else{//如果有新版本.则提示
			...
			// edition:版本号 ,describe : 更新说明 , releaseDate :发布日期,force : 是否强制更新,id : 发布的版本id ;
			}else{
				span.remove();
			}
		}) ;//加载app版本更新
...
//选择产线下拉列表后选择工位.
function selectStation(v){//v:选中产线的值
	if(mui('input[name="stationNo"]').length){
		var i = mui('input[name="stationNo"]')[0],k='SDINTPICKER'+i.attr('name'),o=i.attr('data-options').tojson() || {};
		i.value='';
		i.SDINTPICKER=true ;
		!window[k]&&(window[k]=new mui.PopPicker());
		sendAjax(o.url,{lineName:v},function(data){
			if(o.text){
				for(var t=o.text, a = 0,d;d=data[a];a++){
					d['text']=d[t] ;
				}
			}
			window[k].setData(data);	
		});
	}
}
